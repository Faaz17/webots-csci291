...
// Include the camera API
#include <webots/display.h> 
// Include the display API
...
int main() {
  ...
  // Get the camera and display devices
  // IMPORTANT: You must add 'Camera' and 'Display' nodes to your robot in Webots
  // and name them 'my_camera' and 'my_display' respectively for this code to work.
  WbDeviceTag camera_tag = wb_robot_get_device("CAM");
  //WbDeviceTag display_tag = wb_robot_get_device("my_display");
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    if (image) {
      ...
      // Copy the camera image to the display
    //  wb_display_image_new(display_tag, image, width, height);
    }
